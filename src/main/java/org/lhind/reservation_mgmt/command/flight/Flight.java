...
public class Flight extends AbstractAggregateRoot<FlightId> {
  ...
  /**
   * Business method to cancel a reservation.
   * 
   * @param passengerId
  ...
  public void cancelReservation(PassengerId passengerId)
    ...
    if (confirmedReservations.contains(passengerId)) {
      ...
      apply(new ConfirmedReservationCancelledEvent(flightId, passengerId));

      // Confirm reservation from waitlist if exists
      if (!waitlist.isEmpty()) {
        PassengerId newlyConfirmedPassengerId = waitlist.get(0);
        apply(new ReservationOnWaitlistConfirmedEvent(flightId,
            newlyConfirmedPassengerId));
      }

    } else if (waitlist.contains(passengerId)) {
      // cancel reservation on waitlist
      apply(new ReservationFromWaitlistRemovedEvent(flightId, passengerId));
    } else {
      throw new ReservationDoesNotExistException(flightId, passengerId);
    }
  }
  ...
  private void handle(final ReservationOnWaitlistConfirmedEvent event) {
    waitlist.remove(event.getPassengerId());
    confirmedReservations.add(event.getPassengerId());
  }

  @ApplyEvent
  private void handle(final ConfirmedReservationCancelledEvent event) {
    confirmedReservations.remove(event.getPassengerId());
  }
